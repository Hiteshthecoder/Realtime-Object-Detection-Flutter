...
import 'package:object_detect/view_states/home_view_state.dart';
...
class HomeViewModel extends BaseViewModel<HomeViewState> {
  ...
  HomeViewModel(BuildContext context, this._tensorFlowService) : super(context, HomeViewState(_tensorFlowService.type));
    ...
    if (_isLoadModel && mounted) {
      if (!this._isDetecting && mounted) {
        ...
        if (recognitions != null && mounted) {
          state.recognitions = List<Recognition>.from(recognitions.map((model) => Recognition.fromJson(model)));
