...
import 'package:object_detect/app/app_resources.dart';
import 'package:object_detect/app/app_router.dart';
import 'package:object_detect/app/base/base_stateful.dart';
...
import 'package:object_detect/widgets/aperture/aperture_widget.dart';
import 'package:object_detect/widgets/confidence_widget.dart';
...
class _HomeScreenState extends BaseStateful<HomeScreen, HomeViewModel> with WidgetsBindingObserver {
  ...
  void initCamera() {
    _cameraController = CameraController(cameras[viewModel.state.cameraIndex], ResolutionPreset.high);
  ...
  handleSwitchSource(ModelType item) {
    ...
    Provider.of<NavigationService>(context, listen: false)
        .pushReplacementNamed(AppRoute.homeScreen, args: {'isWithoutAnimation': true});
  }
  ...
  Future<bool> renderedAndSaveImage(Uint8List draw, XFile camera) async {
    ...
    final canvas = new Canvas(recorder,
        Rect.fromPoints(new Offset(0.0, 0.0), new Offset(cameraImage.width.toDouble(), cameraImage.height.toDouble())));
  ...
  AppBar buildAppBarWidget(BuildContext context) {
    return AppBar(
        ...
        PopupMenuButton<ModelType>(
            ...
            itemBuilder: (context) => [
                  PopupMenuItem(
                      ...
                      child: Row(
                        children: <Widget>[
                          Icon(Icons.api, color: !viewModel.state.isYolo() ? AppColors.black : AppColors.grey),
                          Text(' YOLO',
                              style: AppTextStyles.regularTextStyle(
                                  color: !viewModel.state.isYolo() ? AppColors.black : AppColors.grey)),
                        ],
                      ),
                  ...
                  PopupMenuItem(
                      ...
                      child: Row(
                        children: <Widget>[
                          Icon(Icons.api, color: !viewModel.state.isSSDMobileNet() ? AppColors.black : AppColors.grey),
                          Text(' SSD MobileNet',
                              style: AppTextStyles.regularTextStyle(
                                  color: !viewModel.state.isSSDMobileNet() ? AppColors.black : AppColors.grey)),
                        ],
                      ),
                  ...
                  PopupMenuItem(
                      ...
                      child: Row(
                        children: <Widget>[
                          Icon(Icons.api, color: !viewModel.state.isMobileNet() ? AppColors.black : AppColors.grey),
                          Text(' MobileNet',
                              style: AppTextStyles.regularTextStyle(
                                  color: !viewModel.state.isMobileNet() ? AppColors.black : AppColors.grey)),
                        ],
                      ),
                  ...
                  PopupMenuItem(
                      ...
                      child: Row(
                        children: <Widget>[
                          Icon(Icons.api, color: !viewModel.state.isPoseNet() ? AppColors.black : AppColors.grey),
                          Text(' PoseNet',
                              style: AppTextStyles.regularTextStyle(
                                  color: !viewModel.state.isPoseNet() ? AppColors.black : AppColors.grey)),
                        ],
                      ),
      ...
      title: Text(
        AppStrings.title,
        style: AppTextStyles.boldTextStyle(color: AppColors.white, fontSize: AppFontSizes.large),
      ),
    );
  }
  ...
  Widget buildBodyWidget(BuildContext context) {
    ...
    final Size previewSize = isInitialized ? _cameraController.value.previewSize! : Size(100, 100);
    ...
    final maxHeight = screenRatio > previewRatio ? screenHeight : screenWidth * previewRatio;
    final maxWidth = screenRatio > previewRatio ? screenHeight / previewRatio : screenWidth;
                    ...
                    children: <Widget>[
                      ...
                      Consumer<HomeViewModel>(builder: (_, homeViewModel, __) {
                        return ConfidenceWidget(
                          ...
                          previewHeight: max(homeViewModel.state.heightImage, homeViewModel.state.widthImage),
                          previewWidth: min(homeViewModel.state.heightImage, homeViewModel.state.widthImage),
